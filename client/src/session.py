...
class AskServer(Session):
    ...
    def bindDevice(self,id):
        binding_topic = BINDING_CLIENT_TOPIC
        binding_msg = {
            binding_topic : id
        }
        binding_res = BINDING_SERVER_TOPIC + id
        self.client.subscribe(binding_res)
        self.publish(binding_msg)
        start_time = time.time()

        while time.time() - start_time < BINDING_WAITING_TIME:
            if self.recvdMsg(binding_res):
                if self.msgFrame[binding_res] == BINDED_SUCCEED:
                    self.device.append(id)
                    print("绑定设备%s成功"%id)
                else:
                    print("绑定设备%s失败"%id)
                break
